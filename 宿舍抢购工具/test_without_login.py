...
def test_version_without_login(exe_path, version_name, test_count=5):
    """测试单个版本（去除登录耗时）"""
    print(f"\n{'='*80}")
    print(f"测试{version_name}版本（去除登录耗时）")
    print(f"{'='*80}")
    ...
    print(f"\n{version_name}版本统计（去除登录耗时）:")
    print(f"  平均耗时: {avg_time:.3f}秒")
    print(f"  总耗时: {total_time:.3f}秒")
    
    return summary

def run_without_login_test():
    """运行去除登录耗时的测试"""
    print("去除登录耗时的测试")
    print("="*80)
    ...
    for exe_path, version_name in versions:
        ...
        # 测试该版本
        summary = test_version_without_login(exe_path, version_name, test_count=5)
    ...
    print(f"{'版本':<10} {'平均耗时':<15} {'总耗时':<15}")
    print("-" * 40)
    
    for version_name, summary in all_results.items():
        print(f"{version_name:<10} {summary['avg_time']:.3f}秒{'':<10} {summary['total_time']:.3f}秒")
    
    # 保存详细结果
    report_file = f"去除登录耗时测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 运行测试
    results = run_without_login_test()
