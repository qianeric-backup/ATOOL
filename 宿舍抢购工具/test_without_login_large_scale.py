...
def test_version_without_login_large_scale(exe_path, version_name, test_count=20):
    """测试单个版本（去除登录时间）"""
    print(f"\n{'='*80}")
    print(f"测试{version_name}版本（去除登录时间，{test_count}次测试）")
    print(f"{'='*80}")
    ...
    print(f"\n{version_name}版本统计（去除登录时间，{test_count}次测试）:")
...
def run_without_login_large_scale_test():
    """运行去除登录时间的大规模测试"""
    print("去除登录时间的大规模测试（每个版本20次）")
    print("="*80)
    print(f"测试时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"测试版本: v1.1.0, v1.2.4, v1.2.6")
    print(f"测试次数: 每个版本20次")
    print(f"测试说明: 只测试核心功能耗时，不包含登录时间")
    ...
    for exe_path, version_name in versions:
        ...
        # 测试该版本
        summary = test_version_without_login_large_scale(exe_path, version_name, test_count=20)
    ...
    print(f"{'版本':<10} {'平均耗时':<15} {'总耗时':<15} {'最快耗时':<15} {'最慢耗时':<15}")
    print("-" * 70)
    
    for version_name, summary in all_results.items():
        print(f"{version_name:<10} {summary['avg_time']:.3f}秒{'':<10} {summary['total_time']:.3f}秒{'':<10} {summary['min_time']:.3f}秒{'':<10} {summary['max_time']:.3f}秒")
    
    # 保存详细结果
    report_file = f"去除登录时间大规模测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 运行测试
    results = run_without_login_large_scale_test()
    ...
    if results:
        print("[OK] 去除登录时间的大规模测试完成")
        print("  - 测试了v1.1.0、v1.2.4、v1.2.6版本")
        print("  - 每个版本测试了20次")
        print("  - 统计了核心功能耗时（不含登录时间）")
    else:
        print("[FAIL] 去除登录时间的大规模测试失败")
