...
def test_actual_grab_time(exe_path, version_name, test_count=10):
    """测试实际抢购流程耗时"""
    print(f"\n{'='*80}")
    print(f"测试{version_name}版本实际抢购流程耗时（{test_count}次测试）")
    print(f"{'='*80}")
    ...
    print(f"\n{version_name}版本实际抢购流程统计（{test_count}次测试）:")
...
def run_actual_grab_time_test():
    """运行实际抢购流程耗时测试"""
    print("实际抢购流程耗时测试（每个版本10次）")
    print("="*80)
    print(f"测试时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"测试版本: V0.0.9, v1.2.6")
    print(f"测试次数: 每个版本10次")
    print(f"测试说明: 测试实际抢购流程中的耗时，包括登录、查询、校时等")
    ...
    for exe_path, version_name in versions:
        ...
        # 测试该版本
        summary = test_actual_grab_time(exe_path, version_name, test_count=10)
        all_results[version_name] = summary
    
    # 生成对比报告
    print(f"\n{'='*80}")
    print("版本实际抢购流程耗时对比报告")
    print(f"{'='*80}")
    
    print(f"{'版本':<10} {'成功率':<15} {'平均耗时':<15} {'总重试次数':<15}")
    print("-" * 60)
    
    for version_name, summary in all_results.items():
        print(f"{version_name:<10} {summary['success_rate']:.1f}%{'':<10} {summary['avg_time']:.3f}秒{'':<10} {summary['total_retries']}")
    
    # 保存详细结果
    report_file = f"实际抢购流程耗时测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 运行测试
    results = run_actual_grab_time_test()
    ...
    if results:
        print("[OK] 实际抢购流程耗时测试完成")
        print("  - 测试了V0.0.9、v1.2.6版本")
        print("  - 每个版本测试了10次")
        print("  - 统计了实际抢购流程中的耗时")
    else:
        print("[FAIL] 实际抢购流程耗时测试失败")
    
    print("\n测试说明:")
    print("1. 使用演练模式测试，不实际提交验证码")
    print("2. 统计实际抢购流程中的耗时，包括登录、查询、校时等")
    print("3. 对比不同版本的实际使用耗时")
