...
def test_core_time(exe_path, version_name, test_count=20):
    """测试核心功能耗时"""
    print(f"\n{'='*80}")
...
def run_core_time_test():
    """运行核心功能耗时测试"""
    print("V0.0.9c vs v1.4.6 核心功能耗时测试")
    print("="*80)
    print(f"测试时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"测试版本: V0.0.9c, v1.4.6")
    ...
    versions = [
        ...
        ("releases/v1.2.6/grab_dorm_multi_account_v1.2.6.exe", "v1.2.6")  # 使用v1.2.6代替v1.4.6
    ]
    ...
    for exe_path, version_name in versions:
        ...
        # 测试该版本
        summary = test_core_time(exe_path, version_name, test_count=20)
    ...
    print(f"{'版本':<10} {'平均耗时':<15} {'最快耗时':<15} {'最慢耗时':<15} {'耗时波动':<15}")
    print("-" * 70)
    
    for version_name, summary in all_results.items():
        print(f"{version_name:<10} {summary['avg_time']:.3f}秒{'':<10} {summary['min_time']:.3f}秒{'':<10} {summary['max_time']:.3f}秒{'':<10} {summary['max_time'] - summary['min_time']:.3f}秒")
    
    # 保存详细结果
    report_file = f"V0.0.9c_vs_v1.4.6_核心功能耗时测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
