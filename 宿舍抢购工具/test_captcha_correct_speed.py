...
def test_captcha_correct_speed(exe_path, version_name, test_count=10):
    """测试验证码正确速度"""
    print(f"\n{'='*80}")
...
def run_captcha_correct_speed_test():
    """运行验证码正确速度测试"""
    print("验证码正确速度测试")
    print("="*80)
    ...
    for exe_path, version_name in versions:
        ...
        # 测试该版本
        summary = test_captcha_correct_speed(exe_path, version_name, test_count=10)
        if summary:
    ...
    # 保存详细结果
    report_file = f"验证码正确速度测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 运行测试
    results = run_captcha_correct_speed_test()
    ...
    if results:
        print("[OK] 验证码正确速度测试完成")
        print("  - 测试了v1.2.6（修复后）和V0.0.9c版本")
        print("  - 每个版本测试了10次")
        print("  - 只测试验证码正确时的速度")
    else:
        print("[FAIL] 验证码正确速度测试失败")
    
    print("\n测试说明:")
    print("1. 只测试验证码正确时的速度")
