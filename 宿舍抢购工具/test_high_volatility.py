...
"""
高波动对比测试脚本
对比v1.1.0、v1.2.4、v1.2.6版本，各跑5次完整流程
测试耗时、成功率、重试次数
"""
import sys
import os
...
def test_version(exe_path, version_name, test_count=5):
    """测试单个版本"""
    print(f"\n{'='*80}")
    print(f"测试{version_name}版本")
    print(f"{'='*80}")
...
def run_high_volatility_test():
    ...
    for exe_path, version_name in versions:
        ...
        # 测试该版本
        summary = test_version(exe_path, version_name, test_count=5)
    ...
    print(f"{'版本':<10} {'成功率':<15} {'平均耗时':<15} {'平均重试次数':<15}")
    print("-" * 60)
    
    for version_name, summary in all_results.items():
        print(f"{version_name:<10} {summary['success_rate']:.1f}%{'':<10} {summary['avg_time']:.3f}秒{'':<10} {summary['avg_retries']:.2f}")
    
    # 保存详细结果
    report_file = f"高波动对比测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 运行测试
    results = run_high_volatility_test()
    ...
    if results:
        ...
        print("  - 每个版本测试了5次")
        print("  - 统计了耗时、成功率、重试次数")
    else:
        print("[FAIL] 高波动对比测试失败")
