...
def test_core_time_20times(exe_path, version_name, params, test_count=20):
    """测试核心功能耗时"""
    print(f"\n{'='*80}")
...
def run_core_time_20times_test():
    """运行核心功能耗时测试"""
    print("核心功能耗时测试（20次测试）")
    print("="*80)
    print(f"测试时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"测试版本: v1.2.6（修复后）, V0.0.9c")
    print(f"测试次数: 每个版本20次")
    print(f"测试说明: 进一步验证核心功能耗时")
    ...
    for version in versions:
        ...
        # 测试该版本
        summary = test_core_time_20times(version["path"], version["name"], version["params"], test_count=20)
    ...
    # 保存详细结果
    report_file = f"20次核心功能耗时测试报告_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 运行测试
    results = run_core_time_20times_test()
