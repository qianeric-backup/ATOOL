...
def run_core_time_only_test():
    """运行核心功能耗时测试"""
    print("核心功能耗时测试（每个版本20次）")
    print("="*80)
    print(f"测试时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"测试版本: V0.0.9（默认参数 vs 优化参数）")
    print(f"测试次数: 每个参数配置20次")
    print(f"测试说明: 只计算核心功能耗时，不包含登录、查询、校时等")
    ...
    for config in param_configs:
        print(f"\n测试{config['name']}:")
        summary = test_core_time_only(exe_path, f"V0.0.9-{config['name']}", config['params'], test_count=20)
...
def main():
    ...
    if results:
        ...
        print("  - 每个配置测试了20次")
        print("  - 只计算核心功能耗时，不包含登录、查询、校时等")
    else:
        print("[FAIL] 核心功能耗时测试失败")
    
    print("\n测试说明:")
    print("1. 只计算核心功能耗时，不包含登录、查询、校时等")
    print("2. 测试程序启动和参数解析的耗时")
